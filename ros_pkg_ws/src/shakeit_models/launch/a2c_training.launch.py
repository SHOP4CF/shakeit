...
def generate_launch_description():
    ...
    action_dict_path_argument = DeclareLaunchArgument(
        ...
        # place your action spaces in ros_pkg_ws/src/shakeit_models/resource/actions/json_file.json
        # use absolute path as an argument
        default_value='',
    ...
    batch_size_argument = DeclareLaunchArgument(
        'batch_size',
        default_value='10',
        description="Number of actions to take before updating the model."
    )
    ...
    a2c_node = Node(
        ...
        output='screen',
        emulate_tty=True,
        parameters=[{'model_load': model_path,
                     'load': load_model,
                     'save_intervals': save_interval,
                     'action_dict_path': action_dict_path,
